...
export const autocompleteTextInput = (
    ...
    keywords: string[],
    options: AutocompleteTextOptions = {},
): void => {
    ...
    textInput.addEventListener("input", () => {
        ...
        const currentKeyword =
            options.customValueToSearch !== undefined
        ...
        for (const keyword of keywords) {
            ...
            if (
                keyword.substr(0, currentKeyword.length).toUpperCase() ==
                currentKeyword.toUpperCase()
            ) {
                /*create a DIV element for each matching element:*/
                const b = document.createElement("div")
                /*make the matching letters bold:*/
                b.innerHTML = Handlebars.templates[
                    "text_input_autocomplete_div"
                ]({
                ...
                b.addEventListener("click", () => {
                    ...
                    const clickedKeyword =
                        b.getElementsByTagName("input")[0].value
                    textInput.value =
                        options.customValueToSetAfterClick !== undefined
                            ? options.customValueToSetAfterClick(
                                  fullInput,
                                  currentKeyword,
                                  clickedKeyword,
                              )
    ...
    textInput.addEventListener("keydown", (e) => {
        const currentAutoCompleteList = document.getElementById(
            textInput.id + "-autocomplete-list",
        )
    ...
    function closeAutocompleteList(elmnt?: EventTarget) {
        ...
        for (const autoCompleteElement of document.getElementsByClassName(
            "autocomplete-items",
        )) {
